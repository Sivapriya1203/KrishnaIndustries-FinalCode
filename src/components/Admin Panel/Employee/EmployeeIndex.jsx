...
function EmployeeIndex() {
  ...
  const confirmDlt = () => {
    axios
      .delete(`${config.apiUrl}/employee/delete/${dltData}`) // API call to delete employee
      .then((res) => {
        // Update states by filtering out the deleted employee
        setEmpData((prevEmpData) => prevEmpData.filter((emp) => emp.emp_id !== dltData));
        setFilteredEmpData((prevFilteredData) =>
          prevFilteredData.filter((emp) => emp.emp_id !== dltData)
        );
        setOpenDltDialog(false); // Close delete dialog
      })
      .catch((err) => {
        setOpenDltDialog(false); // Close delete dialog
        setOpenErrDialog(true); // Open error dialog
